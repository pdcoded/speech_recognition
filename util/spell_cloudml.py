...
client = storage.Client(project='cloud9-ml')
bucket = client.get_bucket('testing_cloud_ml')
blob = bucket.get_blob('real_batch.zip')
'''
zip_file='arpit_file.zip'
...
'''
try:
    blob.download_to_filename("real_batch.zip")
except Exception as error:
    print(error)

fantasy_zip =zipfile.ZipFile('real_batch.zip')
...
def get_model():
    global MODEL
    if MODEL is None:
        MODEL = kenlm.Model('./real_batch/lm/latest_4ord_poc_train14jun.lm')
    return MODEL
def score(s):
    return sum(prob for prob, _, _ in get_model().full_scores(s))
